...
function removeFirstOccurrences(str, value) {
  ...
  return startIndex !== -1
    ? str
        ...
        .concat(str.substring(startIndex + value.length))
    : str;
}
...
function sumOfCodes(str) {
  return typeof str === 'string'
    ? str.split('').reduce((summa, char) => summa + char.charCodeAt(), 0)
    : 0;
}
...
function isPalindrome(str) {
  const str0 = str
    ...
    .replaceAll('?', '')
    .replaceAll(',', '');
  for (let i = 0; i < Math.ceil(str0.length / 2); i += 1) {
    if (str0[i] !== str0[str0.length - 1 - i]) return false;
  }
  return true;
}
